...
### 6.14 一次完整 Compaction 的端到端流程

把调度、算法和恢复放在同一条时间线上，一次默认 Compaction 可以概括为：

```text
获得最后一条 AssistantMessage
  → 判断是否需要压缩
  → 取得当前 leaf 的完整 Session 路径
  → 确定本次可处理边界
  → 从后向前寻找 firstKeptEntryId
  → 分出历史区、可选 turn prefix、近期保留区
  → 将待摘要消息序列化
  → 调用独立摘要请求
  → 校验摘要完整性
  → 追加 CompactionEntry
  → buildSessionContext() 重建 Agent 消息
  → 下一次请求发送“摘要 + 近期原文 + 新消息”
```
...
| 阶段 | 方法 | 文件 |
|---|---|---|
| 调度 | `_handlePostAgentRun()`、`prompt()` | [`agent-session.ts`](../../packages/coding-agent/src/core/agent-session.ts) |
| 触发判断 | `_checkCompaction()`、`shouldCompact()` | `agent-session.ts`、[`compaction.ts`](../../packages/coding-agent/src/core/compaction/compaction.ts) |
| 准备与切割 | `prepareCompaction()`、`findCutPoint()` | `compaction.ts` |
| 生成摘要 | `compact()`、`generateSummaryWithUsage()` | `compaction.ts` |
| 存储 | `SessionManager.appendCompaction()` | [`session-manager.ts`](../../packages/coding-agent/src/core/session-manager.ts) |
| 当前进程恢复 | `SessionManager.buildSessionContext()` | `session-manager.ts` |
| 重启恢复 | `createAgentSession()` | [`sdk.ts`](../../packages/coding-agent/src/core/sdk.ts) |

```mermaid
sequenceDiagram
    participant A as Agent / Provider
    participant S as AgentSession
    participant C as Compaction Algorithm
    participant L as Summary LLM Call
    participant M as SessionManager

    A-->>S: 最后一条 AssistantMessage + usage/stopReason
    S->>S: _checkCompaction()
    alt 不满足阈值或已禁用
        S-->>A: 正常结束或处理下一条消息
    else manual / threshold / overflow
        S->>M: getBranch()
        M-->>S: root 到 current leaf 的完整路径
        S->>C: prepareCompaction(path, settings)
        C->>C: previous boundary + findCutPoint()
        C-->>S: messagesToSummarize + turnPrefix + firstKept
        S->>S: session_before_compact 扩展事件
        S->>C: compact(preparation)
        C->>L: 独立摘要请求，toolChoice=none
        L-->>C: 完整结构化摘要
        C-->>S: CompactionResult
        S->>M: appendCompaction()
        M->>M: 追加 JSONL 节点并推进 leaf
        S->>M: buildSessionContext()
        M-->>S: Summary + kept messages + post-compaction messages
        S->>S: agent.state.messages = rebuilt messages
        opt overflow 且需要恢复
            S->>A: agent.continue()
        end
    end
...
Compaction 不是定时任务，也不是在每个 token delta 后检查。它在几个明确的稳定边界上发生。

#### 路径一：手动触发

`AgentSession.compact()` 服务于 `/compact`、RPC、SDK 和扩展调用：

1. 先 `await this.abort()`，终止并等待当前 Agent run 结束。
2. 创建专用 `_compactionAbortController`。
3. 发出 `compaction_start(reason="manual")`。
4. 准备、摘要、持久化并重建消息。
5. 发出 `compaction_end`。

手动压缩不会恢复被中断的旧 Agent turn。它的语义是“先停下来，建立一个新检查点”，而不是“压缩后自动续写刚才的响应”。

#### 路径二：Agent run 结束后检查

Assistant Message 在 `message_end` 时被 `_handleAgentEvent()` 保存到 Session，并记录到 `_lastAssistantMessage`。当低层 Agent Loop 发出 `agent_end` 后，`_runAgentPrompt()` 进入 `_handlePostAgentRun()`：

```text
先处理普通可重试错误
  → 再调用 _checkCompaction(lastAssistant)
  → 再决定是否处理 agent_end 扩展新排入的消息
```

如果 `_checkCompaction()` 返回 `true`，外层 `_runAgentPrompt()` 会调用 `agent.continue()`。这个返回值只在 overflow 恢复或压缩期间出现新队列消息时为真；普通 threshold 压缩本身不会重跑一个已经完成的回答。

#### 路径三：发送新 Prompt 前检查

`AgentSession.prompt()` 在构造新 UserMessage 之前，会找到 Agent 状态中最后一条 Assistant Message，并执行：

```text
await _checkCompaction(lastAssistant, false)
```

第二个参数允许检查此前被用户中止的消息。该调用的返回值被刻意忽略，因为接下来应发送用户的新 Prompt，而不是 `continue()` 旧 Assistant Message。

这一入口解决的场景是：上一次 run 因中止、异常 usage 或应用退出，没有在 post-run 阶段形成可用压缩；用户下次输入时系统仍能先整理旧上下文。

### 6.16 `_checkCompaction()` 的判断树
...
├─ no  → 不压缩
└─ yes
   ├─ 当前检查是否应跳过 aborted? → 是则不压缩
   ├─ Assistant 是否早于或等于最新 Compaction 时间? → 是则不压缩
   ├─ 同一模型且 context overflow / recoverable length?
   │  ├─ 响应已成功 stop → overflow 压缩，不重试
   │  ├─ 本次尚未恢复过 → overflow 压缩，最多 continue 一次
   │  └─ 已恢复过一次 → 报告恢复失败，不再循环
   └─ 计算 contextTokens
      ├─ contextTokens > contextWindow - reserveTokens
      │  └─ threshold 压缩，不重试已完成回答
      └─ 否 → 不压缩
```

这里有四个重要保护：

1. **模型一致性保护**：overflow 只在 Assistant Message 的 provider/model 与当前模型一致时处理，避免切换到大窗口模型后仍响应旧模型的 overflow。
2. **压缩时间边界保护**：若 Assistant 比最新 Compaction 更旧，不再用它触发压缩。
3. **单次恢复保护**：`_overflowRecoveryAttempted` 阻止无限 compact-and-retry。
4. **成功响应保护**：`stopReason === "stop"` 时即使判断为 overflow，也只压缩，不重复生成已经成功的答案。
...
例如模型窗口是 128000，`reserveTokens=16384`：

```text
自动压缩阈值 = 128000 - 16384 = 111616
```

- `contextTokens = 111616`：不触发。
- `contextTokens = 111617`：触发 threshold Compaction。

`reserveTokens` 表示希望留给下一次输出、工具调用和估算误差的窗口，不是摘要输出的唯一用途。摘要自身的最大输出预算只是从它派生出来。

### 6.17 第二阶段：上下文 token 如何计算

触发判断尽量使用 Provider 报告的真实 usage，但真实 usage 只覆盖“某次模型请求完成时”的上下文快照。之后可能又追加 ToolResult、Custom Message 或失败消息，所以实现采用“真实锚点 + 尾部估算”。

#### 情况一：当前 Assistant 有有效 usage
...
directContextTokens
= usage.totalTokens
  或 usage.input + usage.output + usage.cacheRead + usage.cacheWrite
```

#### 情况二：当前 Assistant 是 error 或 usage 全零

`estimateContextTokens(messages)` 从后向前找最后一条满足以下条件的 Assistant Message：
...
```text
估计上下文
= 最后一次有效 Provider usage
  + 该 Assistant Message 之后所有消息的 chars/4 估算
```

如果整段历史没有任何有效 usage，则全部使用消息估算。

例如：

```text
Assistant A 的 usage = 90000
之后追加 ToolResult，估算 6000
之后追加 Custom Message，估算 1000
最后 Assistant B error，usage = 0

estimateContextTokens = 90000 + 6000 + 1000 + B 的估算量
```

#### 压缩后为什么会暂时显示 unknown

刚完成 Compaction 时，最近一条有效 Assistant usage 仍然描述压缩前的大上下文。`getContextUsage()` 不会立即信任它；只有最新 Compaction 之后出现新的有效 Assistant usage，UI 才恢复准确百分比。在此之前返回：
...
同理，自动判断会检查 usage 来源是否早于最新 Compaction，防止刚压缩完又被旧 usage 立即触发第二次压缩。

### 6.18 第三阶段：确定本次压缩边界

`prepareCompaction()` 首先处理“从哪里开始重新考虑历史”，而不是立即找切割点。
...
实现找到当前路径上最新的 `CompactionEntry`：

```text
previousSummary = prevCompaction.summary
boundaryStart = prevCompaction.firstKeptEntryId 所在位置
```

若旧 `firstKeptEntryId` 因旧 Session 或迁移异常找不到，才降级为 `prevCompactionIndex + 1`。

边界从旧 `firstKeptEntryId` 开始有两个作用：

1. 上次保留的近期消息在本次淘汰前会进入新摘要。
2. 上一版摘要通过 `previousSummary` 单独参与增量更新，不需要把旧 CompactionEntry 当普通历史再次摘要。

如果当前路径最后一项本身就是 CompactionEntry，`prepareCompaction()` 直接返回 `undefined`，避免没有新消息时反复压缩同一状态。

`tokensBefore` 也不是原始 JSONL 所有条目的体积，而是：

```text
estimateContextTokens(buildSessionContext(pathEntries).messages)
```

即当前真正活动的、已经理解旧 Compaction 边界的消息投影规模。

### 6.19 第四阶段：`findCutPoint()` 怎样寻找切割点

切割算法只负责确定“第一条保留哪个 Entry”。它不在这一阶段调用 LLM。

#### 1. 预先枚举合法切割点

`findValidCutPoints()` 在 `[boundaryStart, boundaryEnd)` 中收集能够独立开始上下文的 Entry：

- User、Assistant。
- BashExecution、Custom Message。
- Branch Summary。
- 能映射为这些消息的其他上下文 Entry。

它明确跳过 CompactionEntry，也不允许 ToolResult 成为切割点。
...
for i = end - 1 downTo start:
    accumulated += estimateTokens(contextMessages(entry[i]))
    if accumulated >= keepRecentTokens:
        cutIndex = 第一个位置 >= i 的合法切割点
        break
```

注意使用 `>=`：一旦近期区域达到目标规模，就停止继续向前扩张。

#### 3. 向前吸收相邻的非上下文元数据

找到切割点后，算法会继续向前包含相邻的 Label、Model Change 等不产生上下文消息的元数据，但在以下位置停止：

- 另一个 CompactionEntry。
- 任何能够映射为上下文消息的 Entry。

这不会增加模型 token，却能避免把与保留节点紧邻的元数据无故留在摘要侧。

#### 4. 判断是否切开了一个 Turn

Turn 起点包括 User、BashExecution、Custom Message、Branch Summary 或 Compaction Summary；Assistant 和 ToolResult 不算 Turn 起点。

若 `firstKeptEntry` 是 Assistant，算法反向寻找它所属 Turn 的起点：

```text
turnStartIndex = 最近的 User-like Entry
isSplitTurn = true
```

如果找不到 Turn 起点，则不会声明 Split Turn，因为无法安全构造“本回合前缀”的语义范围。

### 6.20 第五阶段：把路径分成三块

有了 `CutPointResult` 后，`prepareCompaction()` 构造三类数据。

#### 普通切割

```text
boundaryStart                         firstKept             boundaryEnd
      │                                   │                      │
      ├──── messagesToSummarize ──────────┤──── kept suffix ─────┤
```
...
```text
boundaryStart       turnStart              firstKept          boundaryEnd
      │                  │                      │                   │
      ├─ old history ────┤─ turn prefix ───────┤─ kept suffix ────┤
```

此时：

```text
messagesToSummarize = [boundaryStart, turnStart)
turnPrefixMessages  = [turnStart, firstKept)
kept suffix         = [firstKept, boundaryEnd)
```

CompactionEntry 本身不会进入 `messagesToSummarize`，因为上一版摘要已经通过 `previousSummary` 单独传入。

如果历史区和 turn prefix 都为空，说明没有任何实际可压缩内容，`prepareCompaction()` 返回 `undefined`。

### 6.21 第六阶段：生成摘要请求

`compact()` 根据是否 Split Turn 选择一条或两条摘要请求。

#### 普通切割：生成或更新历史摘要

`generateSummaryWithUsage()` 接收：

- `messagesToSummarize`。
- 可选 `previousSummary`。
- 当前模型、Thinking Level 和认证信息。
- `reserveTokens`、自定义说明和 AbortSignal。

消息先经过 `convertToLlm()`，把 BashExecution、Custom Message 等应用类型转成统一 Message，再由 `serializeConversation()` 转为带角色标签的纯文本。
...
并切换为 update 指令，要求保留旧事实、合并新进展和刷新 Next Steps。

#### Split Turn：两阶段摘要

若切割发生在一个 Turn 中间：

1. 对 `messagesToSummarize` 生成普通历史摘要；若该区域为空，使用 `No prior history.`。
2. 对 `turnPrefixMessages` 使用专门的前缀摘要格式。
3. 把两者合并成单个 summary。
...
这不是单纯为了格式美观。历史摘要回答“整个任务是什么”，Turn Prefix 摘要回答“为什么保留区从半个回合中间开始”。
...
默认 `reserveTokens=16384` 时，普通摘要上限为 13107 tokens，前缀摘要上限为 8192 tokens；如果模型自身 `maxTokens` 更小，则使用模型上限。

#### 独立请求隔离

所有内建摘要调用都经过 `completeSummarization()`：

- `toolChoice: "none"`：摘要模型不能调用工具。
- `cacheRetention: "none"`：不为一次性大摘要写 Prompt Cache。
- 使用独立 routing session ID；调用方未提供时生成新的 UUID。
- 复用当前 Session 的 stream function，使 SDK 的网络、认证和 Provider 行为保持一致。
- 复用 `settings.retry`，只对可重试的临时错误执行有界重试。

### 6.22 第七阶段：把摘要当作原子检查点校验

摘要生成成功不等于文本流中出现过字符。只有满足完整性条件，结果才允许写入 Session：

| 返回情况 | 处理 |
|---|---|
| `stopReason: "stop"` 且为纯文本 | 接受 |
| 临时网络错误 | 按 RetrySettings 有界重试 |
| `stopReason: "error"` | 失败，不持久化 |
| `stopReason: "length"` | 失败，不持久化部分摘要 |
| 返回 ToolCall | 失败，不执行工具，不持久化 |
| AbortSignal 已取消 | 取消，不持久化 |

`stopReason: "length"` 尤其关键。摘要的尾部通常包含 `Next Steps` 和 `Critical Context`，把被截断的前半段当成新检查点会让后续恢复看似成功、实则丢失行动状态。
...
生成失败时不会留下半个 CompactionEntry，旧活动上下文仍然有效。这是 Compaction 的失败原子性。

### 6.23 第八阶段：存储 CompactionEntry

默认摘要生成完成后，还会把累计文件操作作为 XML 附加到 summary，并把结构化列表放入 `details`。最终调用：
...
1. 加入内存 `fileEntries`。
2. 加入 `byId` 索引。
3. 把 `leafId` 推进到新 CompactionEntry。
4. 追加一行 JSONL。

旧消息不会删除，也不会修改 parentId。`firstKeptEntryId` 是 CompactionEntry 指向某个历史祖先的逻辑恢复坐标，而不是树边的替代品。

### 6.24 第九阶段：当前进程立即恢复活动消息

写入 CompactionEntry 后，手动和自动路径都会立即执行：
...
也就是说，压缩后的当前进程不会维护一套特殊的“刚压缩内存格式”，而是马上使用 SessionManager 的标准恢复算法。这能提前暴露持久化与恢复逻辑不一致的问题。

`buildSessionContext()` 分三步：

#### 1. 计算当前树路径

`buildSessionPath()` 从当前 leaf 按 `parentId` 一直回溯到 root，再反转为时间方向。旁支不进入当前上下文。

#### 2. 找到当前路径上最新 CompactionEntry

`buildContextEntries()` 只使用最新 Compaction：

```text
contextEntries
= [latestCompaction]
  + [latestCompaction 之前，从 firstKeptEntryId 开始的祖先条目]
  + [latestCompaction 之后的当前路径条目]
```

物理路径可能是：

```text
E1 → E2 → E3 → E4 → E5 → E6 → E7 → E8 → C1 → E9 → E10
```

若 `C1.firstKeptEntryId = E6`，活动投影是：

```text
C1 Summary → E6 → E7 → E8 → E9 → E10
```

注意 C1 在物理树中位于 E8 之后，但在模型视图中被投影到保留消息之前。Compaction 因此不是树重排，而是读取时构造的虚拟顺序。

#### 3. 把 Entry 映射为 AgentMessage

`sessionEntryToContextMessages()` 将：

- MessageEntry 还原为原消息。
- CustomMessageEntry 还原为 Custom Message。
- BranchSummaryEntry 还原为 Branch Summary Message。
- CompactionEntry 还原为 Compaction Summary Message。
- Model Change、Thinking Change、Label 和普通 Custom Entry 排除出消息数组。

随后 `messages.ts::convertToLlm()` 把 `compactionSummary` 转成一个带边界提示的 User Message：
...
对 Provider 而言，它看到的是合法 User Message，而不是 Coding Agent 的专用 SessionEntry 类型。

### 6.25 第十阶段：进程重启后怎样恢复

进程重启或显式 resume 时，恢复路径与刚压缩后的内存重建基本相同：

```text
SessionManager.open(session.jsonl)
  → loadEntriesFromFile()
  → 必要时执行 Session 版本迁移
  → _buildIndex() 重建 byId 和当前 leaf
  → buildSessionContext()
  → createAgentSession()
...
1. `loadEntriesFromFile()` 逐行读取 JSONL，跳过无法解析的行，并验证 Session Header。
2. `_buildIndex()` 按 Entry ID 建索引；最后一个物理 Entry 成为当前 leaf。
3. `buildSessionPath()` 只恢复 leaf 所在分支。
4. `getSessionContextSettings()` 在完整路径上恢复最新 Thinking Level 和模型；这些设置不受 Compaction 消息投影影响。
5. `buildContextEntries()` 应用最新 Compaction 的 summary 和 `firstKeptEntryId`。
6. `createAgentSession()` 优先从 Session 恢复模型；若模型不可用，选择回退模型并生成提示。
7. SDK 把恢复出的 `messages` 写入新 Agent 状态。
8. 第一次 Provider 请求前，Compaction Summary 再通过 `convertToLlm()` 变成模型可见的摘要 User Message。

这意味着 Session JSONL 同时承担两种责任：

- 保存完整、可审计的历史树。
- 保存足以重建有限活动上下文的 Compaction 检查点。

### 6.26 Overflow 恢复为什么要两次移除失败 Assistant Message

Overflow 路径有一个容易忽视的时序问题：失败 Assistant Message 在触发 `_checkCompaction()` 前已经经过 `message_end`，因此已经写入 Session。
...
目的是让 `agent.continue()` 不从错误 Assistant Message 继续。但是 `prepareCompaction()` 使用的是 `sessionManager.getBranch()`，Session 中仍然保留该失败消息。

CompactionEntry 写入后，`buildSessionContext()` 会从 Session 重建 Agent 状态。如果失败消息位于 `firstKeptEntryId` 之后，它会再次出现在数组尾部。因此在 `willRetry` 时还要第二次检查并移除：

```text
if last message is assistant
   and stopReason is error or length:
       remove it again
```

然后 `_handlePostAgentRun()` 才调用 `agent.continue()`。

这一设计同时满足：

- **运行正确性**：重试上下文不能以失败 Assistant 结尾。
- **审计完整性**：失败响应仍保存在 Session JSONL。
- **恢复有界性**：只 compact-and-retry 一次，第二次失败直接报告。

如果是在新 Prompt 发送前触发，`prompt()` 会完成相同压缩，但忽略 `_checkCompaction()` 的 `true` 返回值，随后创建新的 UserMessage，而不会继续旧响应。

### 6.27 完整数值示例：从切割到下一次恢复

下面使用简化的估算 token 展示 Split Turn。数字用于解释算法，不代表真实 tokenizer 结果。

假设当前路径为：

| Entry | 内容 | 估算 token | 所属 Turn |
|---|---|---:|---|
| E1 | User：分析上下文工程 | 1800 | T1 |
| E2 | Assistant：给出总体计划 | 1200 | T1 |
| E3 | User：深入 Compaction | 1000 | T2 |
| E4 | Assistant：调用 read | 500 | T2 |
| E5 | ToolResult：大量源码 | 6500 | T2 |
| E6 | Assistant：形成中间结论 | 2000 | T2 |
| E7 | User：继续核对测试 | 1500 | T3 |
| E8 | Assistant：测试结论 | 5000 | T3 |
...
E8: 5000
E7: 5000 + 1500 = 6500
E6: 6500 + 2000 = 8500  → 达到预算
```

E6 是合法 Assistant 切割点，所以：

```text
firstKeptEntryId = E6
```

但 E6 属于从 E3 开始的 T2，因此这是 Split Turn：

```text
messagesToSummarize = [E1, E2]
turnPrefixMessages  = [E3, E4, E5]
kept suffix         = [E6, E7, E8]
```

系统分别生成：

1. E1/E2 的任务历史摘要。
2. E3/E4/E5 的 Turn Prefix 摘要。
3. 合并摘要并追加文件列表。
...
```text
物理树：E1 → ... → E8 → C1 → E9 → E10
活动视图：C1 Summary → E6 → E7 → E8 → E9 → E10
```

第二次 Compaction 时：

```text
previousSummary = C1.summary
boundaryStart = E6
```

如果 E6—E8 已不再属于新的近期窗口，它们会作为新消息进入摘要更新；C1 本身不会作为普通消息重复摘要。结果 C2 将包含“C1 旧检查点 + E6 之后的新进展”的合并版本。

### 6.28 Compaction 算法的设计精华

#### 精华一：触发预算与保留预算分离

```text
reserveTokens     → 决定什么时候必须压缩
keepRecentTokens  → 决定压缩后保留多少近期原文
```

这两个参数解决不同问题。前者保护下一轮请求的安全余量；后者控制语义摘要和原始细节之间的比例。把二者合并成一个阈值，会导致“为了早点触发而被迫少保留”或“为了多保留而触发太晚”。

#### 精华二：真实 usage 做锚，局部估算补尾部

完全依赖 tokenizer 会增加 Provider 耦合和计算成本；完全依赖 `chars/4` 又会忽略真实缓存与模型计费语义。Pi 使用最近有效 Provider usage 作为锚点，只估算之后新增的消息，在准确性和通用性之间折中。

#### 精华三：从后向前保留，优先保护近期工作集

Agent 下一步最依赖刚发生的工具结果、错误和决策。反向累计天然实现“近期原文优先”，旧信息则通过摘要保留语义，而不是平均截短每一轮。

#### 精华四：切割点理解消息协议，而不只是 token 数

不能从 ToolResult 开始、Assistant ToolCall 必须与结果保持可解释关系、Split Turn 必须补充原始请求。这些不变量说明压缩算法操作的是对话协议，不是普通字符串数组。

#### 精华五：一个超大 Turn 也能被压缩

只允许在 User Message 处切割会导致单个工具密集型 Turn 大于窗口时无解。允许在 Assistant 切割，并单独总结 Turn Prefix，使算法能够处理“一个回合本身就超过近期预算”的极端情况。

#### 精华六：滚动摘要不会漏掉上一轮保留区

`previousSummary + old firstKeptEntryId` 构成滚动检查点协议：

```text
上版摘要 A + 上版保留 B + 新增 C
  → 新版摘要 A′（吸收应该淘汰的 B/C）+ 新保留 D
```

这解决了普通滑动窗口最容易出现的漏洞：一段消息曾被保留，因此没有进入旧摘要；后来滑出窗口时却被直接丢弃。

#### 精华七：追加式存储，读取时压缩

Compaction 不删除历史，而是追加一个带恢复坐标的检查点。优点是可回退、可审计、可创建新分支；代价是 Session 文件会持续增长，磁盘规模和列表扫描仍需单独治理。

#### 精华八：写入后立即走标准恢复路径

压缩完成后不是手工拼接 Agent 消息，而是调用 `buildSessionContext()`。当前进程和重启进程共用同一恢复算法，减少“运行时看起来正常，但重启后上下文不同”的双实现风险。

#### 精华九：摘要具有失败原子性

长度截断、工具调用、错误和取消都不能产生新 CompactionEntry。只有完整摘要才推进 leaf，使旧检查点在失败期间保持有效。

#### 精华十：错误证据与运行上下文分离

Overflow Assistant Message 保留在 Session 供审计，却从 Agent state 移除以便重试。这延续了整套上下文工程的核心原则：持久化事实和当前执行投影不必相同。

#### 精华十一：摘要调用是隔离的控制面请求

禁用工具、禁用缓存写入、独立 routing ID、有界重试和专用 System Prompt，使摘要不会意外继续原任务、执行副作用或污染正常会话的缓存行为。

#### 精华十二：扩展发生在检查点边界

`session_before_compact` 可以取消或提供完整自定义 CompactionResult；成功保存后 `session_compact` 再通知扩展。Core 仍控制事件顺序、持久化和状态重建，策略扩展不需要复制整个 Agent Loop。

### 6.29 算法的主要权衡与风险

| 设计 | 收益 | 代价或风险 |
|---|---|---|
| `chars/4` 估算 | Provider 无关、速度快 | 对代码、CJK、图片和不同 tokenizer 存在误差 |
| ToolResult 摘要输入限 2000 chars | 避免摘要请求被日志淹没 | 关键错误若在尾部可能缺失 |
| LLM 语义摘要 | 能保留目标、决策和进度 | 可能遗漏、重述错误或逐轮漂移 |
| 保留近期原文 | 近期工具细节准确 | 单个巨大近期 Turn 仍可能昂贵 |
| 追加式 JSONL | 可恢复、可审计、支持分支 | 不回收磁盘历史，文件持续增长 |
| 单次 overflow 重试 | 避免死循环和重复成本 | 一次压缩仍不足时需要用户干预 |
| 扩展可提供自定义结果 | 策略灵活 | 扩展必须保证 `firstKeptEntryId` 和摘要语义有效 |
| 固定结构摘要 | 恢复字段稳定 | 对非编码任务可能显得僵硬 |

因此 Compaction 的正确定位是：

> 它是一个有损、可恢复、协议感知的滚动检查点算法，不是无损存档，也不是通用长期记忆数据库。

代码和文档等重要成果仍应写入工作区；System Prompt 与 Tool Schema 的固定成本仍需独立治理；特别关键的原始证据不应只依赖多轮摘要长期保存。
